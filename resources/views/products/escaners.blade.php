    ...
    <div id="default-carousel" class="relative w-full" data-carousel="slide">
        ...
        <div class="overflow-hidden relative md:h-full xl:h-full 2xl:h-full">
            ...
            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                <img src="{{asset('img/zebraescaners.jpg')}}"
                    class="block absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2">
                <div class="absolute inset-x-[15%] bottom-96  py-5 text-center text-black md:block">
                    <h1 class="text-4xl font-bold">Escaners</h1>
                </div>
            </div>
            <!-- Item 2 -->
            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                <img src="{{asset('img/zebraescaner2.jpg')}}"
                    class="block absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2">
                <div class="absolute inset-x-[15%] bottom-96  py-5 text-center text-black md:block">
                    <h1 class="text-4xl font-bold">Escaners</h1>
                </div>
            ...
            <!-- Item 3 -->
            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                <img src="{{asset('img/zebraescnaer3.jpg')}}"
                    class="block absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2">
                <div class="absolute inset-x-[15%] bottom-96  py-5 text-center text-black md:block">
                    <h1 class="text-4xl font-bold">Escaners</h1>
                </div>
            ...
            <!-- Item 4 -->
            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                <img src="{{asset('img/zebraescaner4.jpeg')}}"
...
<div>
    <div class="m-auto w-3/4">
        <br>
        <p class="text-center">Escaners</p>
        <br>
        <hr class="border-2">
        <br>
        <div class="w-3/4 m-auto b-32 grid grid-cols-auto gap-2 lg:grid-cols-4">
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('escaners','todos')}}">Todos</a>
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('escaners','uso_general')}}">Uso general</a>
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('escaners','industrial')}}">Industrial</a>
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('escaners','montaje_fijo')}}">Montaje Fijo</a>
        </div>
        <br>
    </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-6">
            @foreach($escaners as $escaner)
                @foreach($productos as $producto)
                    @if($producto->id == $escaner->products_id)
                        @php {{$product = $producto;}} @endphp
                    @endif
                @endforeach
                @php
                    $id = 'modal-'.str_replace(' ','',$escaner->nameProduct);
                @endphp
                <button 
                class=" hover:bg-slate-200 text-black font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                data-modal-target="#{{ $id }}"
                >
                    <img class="m-auto" id="{{$escaner->id}}" alt="{{$escaner->nameProduct}}" src="{{asset('uploads'.'/'.$escaner->imagen)}}">
                    {{ $escaner->nameProduct}}
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <div class="modal-content bg-white p-4">
                                ...
                                <div class="columns-2 gap-20 items-center flex">
                                    <div class="p-4">
                                        <img src="{{asset('uploads'.'/'.$escaner->imagen)}}">
                                    </div>
                                    <div>
                                        <!-- Marca del producto -->
                                        <p><a class="text-base font-bold">Marca: </a>{{$escaner->brandProduct}}</p>
                                        <!-- Modelo del producto-->
                                        <p><a class="text-base font-bold">Modelo: </a>{{$escaner->modelProduct}}</p>
