...
<div>
    ...
    <div id="default-carousel" class="relative w-full" data-carousel="slide">
        ...
        <div class="overflow-hidden relative md:h-full xl:h-full 2xl:h-full">
            ...
            <!-- Item 2 -->
            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                <img src="{{asset('img/red2.png')}}" class="block absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2" alt="...">
                <div class="absolute inset-x-[15%] bottom-96  py-5 text-center text-black md:block">
                    <h1 class="text-4xl font-bold">Redes</h1>
                </div>
            </div>
            <!-- Item 3 -->
            <div class="hidden duration-700 ease-in-out" data-carousel-item>
                <img src="{{asset('img/red3.jpeg')}}" class="block absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2" alt="...">
                <div class="absolute inset-x-[15%] bottom-96  py-5 text-center text-black md:block">
                    <h1 class="text-4xl font-bold">Redes</h1>
                </div>
            </div>
...
<div>
    <div class="m-auto w-3/4">
        ...
        <div class="w-3/4 m-auto b-32 grid grid-cols-auto gap-2 lg:grid-cols-6">
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('redes','todos')}}">Todos</a>
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('redes','cableado_estructurado')}}">Cableado Estructurado</a>
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('redes','servidores')}}">Servidores</a>
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('redes','accesorios')}}">Accesorios</a>
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('redes','equipo_wlan_inalambrico')}}">Equipo WLAN / Inalambrico</a>
            <a class="text-center bg-gray-200 hover:bg-gray-400 transition-colors cursor-pointer p-4 font-bold" href="{{route('redes','networking')}}">Networking</a>
        </div>
        <br>
    </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-6">
            @foreach($redes as $red)
                @foreach($productos as $producto)
                    @if($producto->id == $red->products_id)
                            @php {{$product = $producto;}} @endphp
                    @endif
                @endforeach
                @php
                    $id = 'modal-'.str_replace(' ','',$red->nameProduct);
                @endphp
                <button 
                class=" hover:bg-slate-200 text-black font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                data-modal-target="#{{ $id }}"
                >
                    <img class="m-auto" id="{{$red->id}}" alt="{{$red->nameProduct}}" src="{{asset('uploads'.'/'.$red->imagen)}}">
                    {{ $red->nameProduct}}
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <div class="modal-content bg-white p-4">
                                ...
                                <div class="columns-2 gap-20 items-center flex">
                                    <div class="p-4">
                                        <img src="{{asset('uploads'.'/'.$red->imagen)}}">
                                    </div>
                                    <div>
                                        <!-- Marca del producto -->
                                        <p><a class="text-base font-bold">Marca: </a>{{$red->brandProduct}}</p>
                                        <!-- Modelo del producto-->
                                        <p><a class="text-base font-bold">Modelo: </a>{{$red->modelProduct}}</p>
